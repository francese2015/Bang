...
if [ "x$TITANIUM_CLI_XCODEBUILD" == "x" ]; then
    /usr/local/bin/node "/Users/johneisenheim/.appcelerator/install/4.0.0/package/node_modules/appc-cli-titanium/node_modules/titanium/bin/titanium" build --platform iphone --sdk "4.0.0.v20150518154210" --no-prompt --no-progress-bars --no-banner --no-colors --build-only --xcode
    exit $?
else
